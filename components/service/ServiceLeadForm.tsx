...
export default function ServiceLeadForm({
  ...
  const { servico, desafioLabel, desafioPlaceholder, checkbox } = config;
  ...
  const handleOpcao = (value: string) => {
    startOnce();
    setForm((prev) => ({
      ...prev,
      opcoes: prev.opcoes.includes(value)
        ? prev.opcoes.filter((v) => v !== value)
        : [...prev.opcoes, value],
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (checkbox && form.opcoes.length === 0) {
    ...
    try {
      const res = await fetch("/api/leads", {
        ...
        body: JSON.stringify({
          ...
          url_params: {
            servico,
            ...(checkbox ? { opcoes: form.opcoes.join(", ") } : {}),
            ...urlParams,
          },
        }),
      });
  ...
  if (success) {
    return (
      <div className="w-full max-w-lg mx-auto text-center py-10 px-6 rounded-2xl border border-[rgba(147,51,234,0.3)] bg-[rgba(147,51,234,0.08)]">
        ...
        <p className="text-violet-200/70 text-lg">{successMessage}</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
            ...
            {checkbox.options.map((op) => (
              <label key={op} className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className={`text-sm transition-colors ${
                  form.opcoes.includes(op) ? "text-white font-medium" : "text-violet-200/60 group-hover:text-violet-200"
                }`}>{op}</span>
              </label>
            ))}
          </div>
      ...
      <div>
        <label htmlFor="desafio" className={labelClass}>{desafioLabel}</label>
        <textarea
          id="desafio"
          name="desafio"
          required
          ...
          onChange={handleChange}
          rows={3}
          placeholder={desafioPlaceholder}
